...
\block{5.\ R\'esultats -- Comparaison}{
  ...
  \begin{tikzpicture}
    ...
    \addplot[fill=alert!60, draw=alert!80] coordinates {(1,.300)};
    \addplot[fill=secondary!65, draw=primary!70] coordinates {(2,.743)(3,.754)};
    \addplot[fill=orangecol!50, draw=orangecol!80] coordinates {(4,.706)};
    \addplot[fill=accent!65, draw=accent!80] coordinates {(5,.757)(6,.760)};
    \end{axis}
  \end{tikzpicture}
}
...
\block{6.\ D\'ecouvertes Scientifiques}{
  ...
  \vspace{0.3cm}
  %% Deux captures côte à côte (légèrement plus grandes qu'avant)
  \begin{tabular}{@{}p{0.485\linewidth}@{\hspace{0.03\linewidth}}p{0.485\linewidth}@{}}
    ...
    \centering\small\textit{Sant\'e historique} &
    \centering\small\textit{Pollution temps r\'eel} \\
  \end{tabular}
}
